...
- Ne pas supprimer localement les scripts `data/kpi_perfect/reporting_sql/*.sql` ni les fichiers `definition/*.tmdl`/`visual.json` sous pretexte de nettoyage : ils restent utiles pour l'audit, la reprise et les tests, meme lorsqu'ils ne sont plus versionnes. La documentation Power BI est centralisee dans `documentation_web/perfect_power_bi/`.
