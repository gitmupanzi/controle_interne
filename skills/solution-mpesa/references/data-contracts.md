...
- Ne jamais utiliser `reference_id` seul comme grain d'un événement crédit. Séparer au minimum par client, devise et `created_at`, puis par `ref_no` lorsqu'il existe. Référence de non-régression : `LN11FAEGXL` comporte 12 écritures d'octroi à `2026-07-22 16:17:16` et 4 écritures de remboursement portant ce `reference_id` à `2026-07-22 16:26:43`. Deux lignes techniques supplémentaires `NORMAL SAVINGS`/`FIXED SAVINGS` sans ce `reference_id` portent le même `ref_no` de remboursement; l'événement de remboursement contient donc 6 écritures.
- Réserver le solde du portefeuille M-PESA aux analyses internes lorsqu'il serait présenté comme bloc séparé. Dans l'aperçu, le Word et le PDF, fusionner les flux externes, les bornes d'ouverture/clôture observées et la situation de l'épargne dans `Synthèse financière par devise`, avec `Compte ouvert` et `Compte bloqué` issus de `Savings Account`, sans compenser ces positions avec le crédit.
- La `vue Bisou Bisou` et la `vue client` sont deux restitutions distinctes. La `vue Bisou Bisou` explique les flux de l'institution. La `vue client` explique la situation du client : `Position épargne = Compte ouvert + Compte bloqué`, avec prêt net reçu, remboursements et frais/intérêts de crédit présentés séparément. Dans la `vue client`, ne jamais afficher `Cumul net des flux` comme solde client. Dans le contenu visible des Word/PDF, ne jamais afficher `vue client`, `vue Bisou Bisou`, `point de vue Bisou Bisou` ou `Synthèse de la position client`; conserver les titres neutres `Extrait de compte`, `Extrait minimal de compte` et `Synthèse financière par devise`.
...
- Commencer la restitution financière Word/PDF par le titre sobre `Synthèse financière par devise`, suivi d'un tableau limité à `Devise`, `Ouverture`, `Entrées externes`, `Sorties externes`, `Flux net externe`, `Clôture`, `Compte ouvert` et `Compte bloqué`. Ne pas ajouter `point de vue Bisou Bisou` au titre ni de phrase explicative sous le tableau. Ne pas produire de tableau séparé `Situation de l'épargne` ou `Situation financière actuelle du client`; l'ouverture et la clôture restent dans la synthèse.
...
- Le nom du Word Turbo seul suit `extrait_compte_<customer_id>_<telephone>_<devise>_<debut>_<fin>.docx`. Avec G2 chargé, insérer `Nom_client` entre `customer_id` et le téléphone. Le contenu du Word officiel ne porte pas de suffixe `[Turbo]`. Dans le détail, nommer `operation_reference` `Référence Turbo` et la dernière colonne `Cumul net des flux`; réserver `Receipt No.` au rapprochement G2. Ce cumul exclut toujours l'ouverture client et ne constitue jamais un solde réel; l'ouverture et la clôture sont affichées dans `Synthèse financière par devise`.
...
- Lorsqu'un remboursement est financé par `NORMAL SAVINGS`, le conserver dans `Remboursements observés` mais l'exclure des entrées externes, des sorties externes et du détail transactionnel. Il s'agit d'un transfert interne entre l'épargne ouverte et le crédit, pas d'une nouvelle trésorerie reçue.
- Produire `elements_extrait_client_turbo` et sa synthèse avec exactement sept familles : `Depot normal`, `Dépôt à terme (DAT)`, `Retrait`, `Remboursement d'un credit depuis le compte M-PESA`, `Remboursement d'un credit depuis le compte ouvert`, `Retour du capital mis en DAT` et `Entree des interets du capital mis en DAT`.
- Construire `Dépôt à terme (DAT)` depuis les événements `Sortie M-PESA_Turbo vers DAT`. Dans les Word/PDF, conserver ce mouvement dans les totaux et cumuls, le présenter dans `Éléments couverts par l'extrait client`, puis l'omettre du tableau `Détail des transactions` pour éviter une double restitution. Ne pas le transformer en position `DAT en cours` sans `Savings Account`.
...
- `Clients Turbo actifs` correspond aux clients ayant au moins une operation Transactions [Turbo] dans la periode. Dans `vue_ensemble`, conserver le compteur global et ajouter le compteur par devise afin de ne pas répéter un total global comme s'il était propre à CDF ou USD. `Clients Turbo connus` vient de `Customers [Turbo]` quand la source est chargee; sinon il est degrade depuis les clients observes dans les sources Turbo disponibles.
...
- L'Extrait client affiche le solde d'ouverture et la clôture dans `Synthèse financière par devise` lorsqu'ils peuvent être observés depuis Turbo ou saisis dans l'onglet client. Son `Cumul net des flux` commence toujours à zéro et suit exclusivement les entrées et sorties du point de vue Bisou Bisou.
...
- Le Word et le PDF de l'Extrait client utilisent l'A4 portrait. Conserver un seul tableau DAT et un seul tableau de remboursements avec les colonnes compactes contractuelles; compacter le détail transactionnel et autoriser le renvoi à la ligne de la description. Ne pas appliquer cette orientation à l'annexe G2, dont le tableau contractuel reste en paysage.
- Le Word et le PDF distinguent les flux externes, l'ouverture/clôture observées et la situation de l'épargne dans les colonnes du tableau unique `Synthèse financière par devise`, puis présentent les remboursements observés dans leur bloc dédié pour le format complet. Ils ne présentent aucun solde du portefeuille M-PESA comme bloc séparé et ne compensent jamais épargne, DAT et crédit.
- Le PDF minimal de l'Extrait client garde seulement l'en-tête, les critères, `Synthèse financière par devise` avec ouverture/clôture, puis `Détail des transactions`. Les blocs `Éléments couverts par l'extrait client`, `DAT en cours`, `Remboursements observés`, retours DAT et intérêts DAT crédités restent réservés au PDF complet.
- Les exports `vue client` existent en Word et PDF, en format complet et minimal. Le complet inclut la synthèse financière par devise, les DAT en cours utiles et le détail des transactions client. Le minimal inclut l'en-tête, les critères, la synthèse financière par devise et le détail des transactions client. Le scénario Benjamin `243821064833` attend une position épargne finale de 10,00 USD, et non 10,35 USD, car les 0,35 USD sont des frais/intérêts de crédit.
- Tout bloc `DAT en cours` présenté dans un export de l'Extrait client doit inclure `Jours restants`, quel que soit le format complet, la perspective d'export ou la feuille Excel `DAT_En_Cours`.
...
- Le Word et le PDF de l'Extrait client ajoutent avant le détail transactionnel la synthèse des sept familles attendues, `DAT en cours`, `Remboursements observés`, `Retours du capital mis en DAT` et les `Entrées des intérêts du capital mis en DAT` réellement constatées. Le taux DAT apparaît dans les critères. L'Excel client utilise `DAT_En_Cours`, `Remboursements_Turbo`, `Elements_Extrait_Turbo` et `Interets_DAT_Credites`; il n'exporte plus l'ancienne feuille `Interets_DAT_Echus` ni les feuilles de crédit.
