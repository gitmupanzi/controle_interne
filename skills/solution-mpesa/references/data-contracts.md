...
Règles complémentaires de contrôle :

- `Transactions` conserve son rôle de flux, d'historique et d'extrait client. Les montants agrégés de volume ne sont pas affichés dans Clients, Épargnes ou Statistiques; les nombres d'opérations, événements consolidés et lignes détaillées restent disponibles.
- Une ligne `Open Savings` est signalée comme potentiellement incomplète uniquement si `balance = 0`, `status = Active`, `product_name = Open Savings` et `created_at = updated_at`. Elle reste dans le snapshot maître et les encours bruts, avec un tableau séparé dans Clients et Épargnes.
- Ces comptes Open Savings potentiellement incomplets ne doivent pas compter comme produit client dans les KPI décisionnels, `Clients_Sans_Produit`, les présences de produit, les synthèses opérationnelles et les opportunités. Ils sont conservés pour audit qualité afin d'identifier les parcours d'ouverture non finalisés sans gonfler artificiellement la base active.
- Cette classification est un signal de revue. Elle ne prouve ni une anomalie définitive, ni une fraude, ni l'absence de création du client.
...
## Statistiques Solution Numerique

- `Statistiques` est un cockpit operationnel Solution Numerique-first distinct de `Finance et comptabilite`. Il sert a suivre les comptes clients, les produits d'epargne ouverte, les produits DAT / comptes bloques et les produits credit a partir des sources de stock fiables.
- L'ecran `Statistiques` est classe par blocs decisionnels repliables/depliables : `Clients`, `Epargnes et DAT` et `Credits`. Chaque bloc regroupe ses KPI, graphiques et tableaux afin de faciliter la lecture par la direction et d'eviter une page trop longue.
- Les sources sont hierarchisees ainsi : `Savings Account [Solution Numerique]` indispensable pour les encours epargne/DAT, interets IMF, interets Vodacom et frais; `Loans Account [Solution Numerique]` indispensable pour les encours credit, interets, frais, penalites et PAR; `Customers [Solution Numerique]` important pour les comptes clients; `Transactions [Solution Numerique]` utile pour l'historique, les extraits clients, les controles et le comptage d'activite, mais non fiable pour les volumes globaux lorsqu'il est plafonne; `Transactions [G2]` facultatif utile pour l'identite et le rapprochement; `Clients_Perfect` facultatif analytique.
...
- Les champs `Date de debut` et `Date de fin` definissent ensuite une periode plus fine a l'interieur de la vue annuelle. Le Word Statistiques restitue `Perimetre annuel` dans le tableau des criteres. Un instantane unique ne permet pas de reconstituer un solde historique a la cloture d'une ancienne annee.
...
- Ne pas utiliser `Transactions [Solution Numerique]` pour piloter les volumes monetaires globaux, le chiffre d'affaires observe, les tops clients par volume ou les previsions de volume. Les exports Transactions peuvent etre limites aux 100 000 premieres lignes; ces montants seraient donc partiels et peuvent induire la Direction en erreur.
...
- Les comparaisons annuelles N/N-1 ne sont plus prioritaires dans le rapport operationnel. Les conserver uniquement pour une analyse specialisee, apres validation de la couverture des sources.
...
- Les comptes clients actifs et les operations sont calcules depuis les evenements consolides Transactions [Solution Numerique]. Dans les rapports Word, `Numéros clients distincts` compte les numéros normalisés ayant au moins une transaction dans la période filtrée; cet indicateur transactionnel reste séparé de `Nombre de numéros de téléphone observés sur la période`, qui exige à la fois une création dans `Customers.created_at` et une transaction dans le même intervalle. Les nouveaux numeros clients utilisent `Customers.created_at`, sinon la premiere observation connue du compte client. Les nouveaux produits d'epargne ouverte et DAT utilisent `date_activated`, puis `date_approved`, puis `created_at`. Les nouveaux produits credit utilisent `Loans Account.created_at`.
- Les encours epargne ouverte, encours DAT, encours credit, interets, frais, penalites et PAR restent strictement separes par `currency_code`. Les nombres de comptes clients, produits, credits ou operations peuvent etre presentes globalement.
...
- L'export Word du rapport statistiques inclut le logo, les criteres, une synthese executive, puis un rapport professionnel par blocs : `Clients`, `Epargnes et DAT` et `Credits`. Les analyses textuelles recurrentes sont accompagnees de tableaux synthetiques compacts : indicateur operationnel, explication, devise, valeur et nature. Les tableaux doivent eviter les cellules vides nombreuses; lorsqu'un indicateur n'a pas la meme structure qu'un autre, le restituer sur une ligne indicateur-valeur plutot que dans une grille large. Chaque bloc conserve des commentaires textuels courts pour guider la lecture des decideurs. Le Word reprend exactement le perimetre filtre de l'onglet. Aucun KPI ecran ni bloc Word ne doit totaliser les montants, volumes, soldes ou credits entre CDF et USD; seuls les compteurs non monetaires peuvent etre consolides globalement.
...
- Le détail des transactions téléchargé dans le rapport Solution Numérique / M-Pesa ne contient pas `statut_rapprochement` (`Controle Turbo/G2`) : retirer cette colonne du Word et des feuilles Excel `Rapport_Journalier_Detail` et `Rapport_G2_Detail`. Conserver la colonne dans les données internes et les feuilles d'audit, sans modifier les lignes, les montants ou les contrôles.
