...
- Les comptes clients actifs et les operations sont calcules depuis les evenements consolides Transactions [Turbo]. Les nouveaux comptes clients utilisent `Customers.created_at`, sinon la premiere observation Turbo. Les nouveaux produits d'epargne ouverts et DAT utilisent `date_activated`, puis `date_approved`, puis `created_at`. Les nouveaux produits credit utilisent `Loans Account.created_at`; les remboursements utilisent les evenements Turbo.
