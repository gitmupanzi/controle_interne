...
- Dans le relevé, ne jamais utiliser `Cumul net des flux` comme solde client. Le solde du détail suit uniquement le compte ouvert (`NORMAL SAVINGS`), tandis que `Compte ouvert` et `Compte bloqué` viennent de `Savings Account`. Pour le scénario Benjamin `243821064833`, le relevé attend 0,00 USD en compte ouvert et 10,00 USD en compte bloqué; le prêt de 4,65 USD et les frais/intérêts de 0,35 USD ne doivent pas créer un solde de compte ouvert.
...
- Organiser les sous-onglets principaux dans cet ordre : `Importation`, `Extrait client`, `Finance Turbo`, `DAT`, `G2 / DAT`, `Detail des credits`, `Perfect_client`, `Statistiques`.
- Fusionner `Controle des donnees` dans `Importation` : l'onglet d'importation affiche les fichiers charges, les colonnes attendues/manquantes, la composition Savings Account, les controles de donnees et les anomalies Transactions [Turbo].
...
- Calculer `Clients Turbo actifs` depuis les operations Transactions [Turbo] de la periode. Calculer `Clients Turbo connus` depuis `Customers [Turbo]` lorsqu'il est charge, sinon depuis les clients observes dans les sources Turbo disponibles. Construire la courbe de croissance depuis `Customers [Turbo].created_at` lorsque disponible, sinon depuis la premiere observation Turbo.
...
- Proposer dans `Statistiques` un export Word du rapport statistique avec logo, criteres, synthese executive et analyse textuelle professionnelle par blocs : `Clients`, `Comptes ouverts et comptes bloques`, `Credits` et `Transactions`. Le rapport doit privilegier les chiffres, les ratios et les pourcentages lisibles par la direction, sans graphiques dans le Word. Il reprend exactement le perimetre filtre de l'onglet et conserve les annexes utiles : sources, vue d'ensemble et definitions. Les KPI ecran et le Word ne doivent jamais totaliser les montants, volumes, soldes, credits ou chiffre d'affaires entre CDF et USD; seuls les comptes de clients ou d'operations peuvent etre consolides globalement.
