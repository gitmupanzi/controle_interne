...
- Proposer dans `Statistiques` un export Word du rapport statistique avec logo, criteres, synthese executive et analyse textuelle professionnelle par blocs : `Clients`, `Comptes ouverts et comptes bloques`, `Credits` et `Transactions`. Le rapport doit privilegier les chiffres, les ratios et les pourcentages lisibles par la direction, sans graphiques dans le Word. Il reprend exactement le perimetre filtre de l'onglet et conserve les annexes utiles : sources, vue d'ensemble et definitions.
