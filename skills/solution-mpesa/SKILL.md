...
- Conserver deux perspectives d'export distinctes dans `Extrait client` :
  - `vue Bisou Bisou` : restitution interne des flux observés par Bisou Bisou, avec le prêt net versé en sortie et les intérêts/frais de crédit séparés;
  - `vue client` : restitution professionnelle orientée client, où la position finale du client est `Compte ouvert + Compte bloqué`, tandis que le prêt net reçu, les remboursements et les frais/intérêts de crédit restent séparés.
- Dans le contenu visible des Word/PDF, ne jamais afficher les mentions `vue client`, `vue Bisou Bisou` ou `point de vue Bisou Bisou`. Les deux perspectives se distinguent par le nom du fichier, le bouton d'export ou le contexte Streamlit, pas par le texte du relevé. Le titre visible reste `Extrait de compte` ou `Extrait minimal de compte`.
- Dans la `vue client`, ne jamais utiliser `Cumul net des flux` comme solde client. Nommer la dernière position `Position épargne` et la calculer uniquement depuis les mouvements d'épargne/DAT et les soldes `Savings Account`. Pour le scénario Benjamin `243821064833`, la vue client attend 0,00 USD en compte ouvert, 10,00 USD en compte bloqué, 10,00 USD de position épargne, 4,65 USD de prêt net reçu et 0,35 USD de frais/intérêts de crédit.
...
- Proposer aussi les exports `vue client` en Word et PDF, chacun en format complet et minimal. Le format complet garde la synthèse financière par devise, les DAT en cours pertinents et le détail des transactions client. Le format minimal garde l'en-tête, les critères, la synthèse financière par devise et le détail des transactions client. Aucun export `vue client` ne doit afficher `Cumul net des flux`, `vue client` ou `Synthèse de la position client` dans le document.
- Dans tous les exports de l'Extrait client qui affichent un bloc `DAT en cours`, inclure obligatoirement la colonne `Jours restants`, y compris dans les exports complets de la `vue client` et dans la feuille Excel `DAT_En_Cours`.
