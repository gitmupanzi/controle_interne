...
- Organiser les sous-onglets principaux dans cet ordre : `Importation`, `Extrait client`, `Finance Turbo`, `DAT`, `G2 / DAT`, `Detail des credits`, `Perfect_client`, `Statistique`.
...
- Construire `Statistique` comme un cockpit Turbo-first distinct de `Finance Turbo` : analyses clients, activite, croissance, volume, chiffre d'affaires observe, epargne/DAT, credit et top clients. L'ecran doit etre organise en blocs decisionnels professionnels repliables/depliables : `Clients`, `Comptes ouverts et comptes bloques`, `Credits` et `Transactions`, avec les KPI, graphiques et tableaux rattaches a leur famille metier. Les filtres professionnels minimaux sont `Date de debut`, `Date de fin`, `Frequence`, `Devises affichees` et `Top clients affiches`; ils doivent influencer les KPI, graphiques, tableaux et exports.
- Dans `Statistique`, classer l'importance des sources ainsi : `Transactions [Turbo]` indispensable, `Savings Account [Turbo]` indispensable, `Loans Account [Turbo]` tres important, `Customers [Turbo]` important, `Transactions [G2]` facultatif utile, `Clients_Perfect` facultatif analytique.
...
- Proposer dans `Statistique` un export Word du rapport statistique avec logo, criteres, sources, vue d'ensemble, chiffre d'affaires observe, croissance clients, epargne/DAT, credit et definitions. Le Word reprend exactement le perimetre filtre de l'onglet.
