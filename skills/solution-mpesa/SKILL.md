...
- Couvrir dans les Word et PDF client les six familles attendues : dépôt normal, retrait, remboursement depuis le compte M-PESA, remboursement depuis le compte ouvert, retour du capital DAT et entrée des intérêts DAT. Construire les cinq premières depuis les événements Transactions Turbo; construire l'intérêt constaté exclusivement depuis `Savings Account.interest_earned` sur un DAT dénoué.
...
- Dans les Word et PDF client, placer avant `Détail des transactions` la synthèse des six familles, `DAT en cours`, `Remboursements observés`, `Retours du capital mis en DAT` et, lorsqu'elles existent, `Entrées des intérêts du capital mis en DAT`. Le bloc DAT en cours conserve la date de situation Turbo, le `savings_id`, la durée, la souscription, l'échéance, les jours restants, la devise, le capital bloqué, le taux, l'intérêt estimé, capital + intérêt estimé et la situation.
- Dans les Word et PDF client, restituer pour les remboursements la date, la référence Turbo, la devise, le montant payé, le principal remboursé, les intérêts, les pénalités et l'origine `Compte M-PESA` ou `Compte ouvert`. Ne pas afficher les décaissements ni une section crédit.
