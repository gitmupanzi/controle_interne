...
- Pour la section `2. PORTEFEUILLE CLIENT` de ce canevas, considérer le portefeuille comme le stock global des clients actifs à `@date_fin`, et non comme la liste des seuls clients ayant un mouvement ou un compte avec solde. Le nombre doit partir de `ADHERENTS` via `extra_clients_view` : `IND` pour les personnes physiques, et les autres catégories couvertes par la vue (`GROUPES`, `ENTREPRISES`, `IMFS`, etc.) pour les personnes morales. Les soldes de comptes servent uniquement à renseigner le volume financier lorsque disponible ; ils ne doivent pas filtrer la population cliente.
...
- Pour le cycle Money Provider, considérer `OPERATIONS_MOB_WS`, `MOB_OPERATIONS` et `MOB_API_OPERATIONS` comme les trois sources opérationnelles possibles. Les opérations API doivent être rapprochées avec `OPERATIONS_API`; les anciennes opérations mobiles avec `OPERATIONS`. Lorsque `MOB_API_OPERATIONS` alimente les requêtes 119 ou 134, utiliser les comptes disponibles (`DESTINATAIRE` côté client, `NUMERO_COMPTE` côté provider) et signaler les lignes non rattachées dans `controle_qualite` plutôt que les supprimer.
