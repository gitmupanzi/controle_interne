...
def render_surveillance_tab(df: pd.DataFrame) -> None:
    ...
    render_panel_title("Surveillance operationnelle")
    render_summary_box(
        "Lecture de surveillance",
        [
            "Cet onglet regroupe les blocs d'action, les classements actifs et les dossiers a suivre en priorite.",
            "La synthese du haut reste reservee aux KPI standard et aux graphiques standard",
            f"{snapshot['high_risk_count']:,}".replace(",", " ")
            + " dossiers sont en risque eleve et "
            + f"{snapshot['overdue_30_count']:,}".replace(",", " ")
            + " depassent 30 jours de retard.",
        ],
    )
    ...
    with snapshot_col:
        ...
        render_summary_box(
            "Points de controle immediats",
            [
                f"Capacite negative : {snapshot['negative_capacity_count']:,}".replace(",", " ")
                + " dossiers a verifier rapidement.",
    ...
    with ranking_left:
        ...
        if not agency_summary.empty:
            ...
            st.info("Aucune synthese agence n'est disponible sur le perimetre courant.")
    ...
    with ranking_right:
        ...
        if not product_summary.empty:
            ...
            st.info("Aucune synthese produit n'est disponible sur le perimetre courant.")

    watchlist = build_watchlist(df)
    render_panel_title("Dossiers a suivre en priorite")
    if not watchlist.empty:
        st.dataframe(watchlist.head(20), width="stretch", hide_index=True)
    else:
        st.info("Aucun dossier prioritaire n'a ete detecte avec les regles de surveillance actuelles.")

    render_panel_title("Apercu des dossiers")
