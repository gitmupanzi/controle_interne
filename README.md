...
- Le bloc `Crédit et remboursements observés [Turbo]` ventile le montant versé au client, la dette créée observée, l'intérêt comptabilisé, le principal remboursé, les pénalités et les remboursements associés à l'épargne ou au DAT. Ces montants décrivent les écritures du fichier et ne constituent pas un taux annuel ou une rentabilité nette.
- Le bloc `Positions observées par produit [Turbo]` reprend le dernier `bal_after` exploitable pour l'épargne courante, le DAT et le principal du crédit. Il compare ce montant aux fichiers `Current Savings`, `Fixed Savings` et `Loans` lorsqu'ils sont chargés. Sans confirmation, le montant reste libellé `solde observé`, jamais `solde officiel`.
...
- Le parcours, le comportement, la ventilation crédit, les positions et les contrôles utilisent le même filtre de devise, période, référence et type d'opération que l'extrait. Dans le Word client, les tableaux `Synthèse du comportement observé`, `Positions observées et rapprochement des soldes` et `Jalons du parcours financier` sont retirés; `Détail des transactions` reste présent et le pied de page indique `Solution Bisou Bisou Digital`.
...
Dans `Extrait client`, le classeur contient les feuilles non vides parmi `Synthese`, `Extrait_Turbo`, `Parcours_Turbo`, `Credit_Client_Turbo`, `Positions_Turbo`, `Comportement_Turbo`, `Mouvements_Internes`, `Controles_Client_Turbo`, `DAT_Final`, `Credits`, `G2_DAT` et `Diagnostics`. `G2_DAT` est facultative et limitée au client sélectionné.
