...
Dans l'interface, l'ancienne appellation `Turbo` est désormais présentée comme `Solution Numérique`. Le nom `Turbo` reste conservé dans le code, les noms de colonnes, certaines feuilles d'export et les clés de cache lorsqu'il sert de contrat technique historique. Le canal métier reste M-Pesa; `G2` désigne le rapport de contrôle M-Pesa, et non une source financière à additionner séparément.

Les deux synthèses `Customers with Current Savings Account` et `Customers with Fixed Savings Account` n'ont plus d'emplacements séparés. Elles peuvent être sélectionnées ensemble dans `Savings Account [Solution Numérique]` comme solution de compatibilité. Ce mode couvre seulement les comptes à solde positif et ne reconstitue ni les comptes à solde nul ni l'historique exhaustif. Si le fichier complet et les synthèses sont chargés ensemble, le fichier complet est prioritaire et les synthèses sont ignorées pour éviter tout doublon.
...
Le fichier `Rapport G2 M-Pesa` est facultatif pour ouvrir le sous-onglet `Solution Numérique / M-Pesa`. En son absence, le mode Solution Numérique seule reconstruit uniquement les opérations démontrables dans `Transactions M-PESA_Turbo` : dépôt normal, DAT et remboursement regroupés par `ref_no`, puis retraits `Retrait Vers M-Pesa` regroupés par `reference_id + created_at`. `created_at` fournit alors la date et l'heure. Le mode ne fabrique ni nom G2, ni statut G2, ni solde G2, ni date d'initiation/finalisation G2; les contrôles croisés rapport G2/Solution Numérique sont affichés comme `Non applicable - Turbo seul`. Lorsqu'il est chargé, le rapport G2 reste la source de son propre relevé de contrôle, mais la Solution Numérique demeure la source financière principale de la Solution M_PESA.
...
- Le bloc `Remboursements observés [Turbo]` retient uniquement les événements de remboursement présents dans Transactions M-PESA_Turbo et correspondant aux filtres actifs. Il restitue la date, la référence, la devise, le montant payé, le principal remboursé, les intérêts, les pénalités et l'origine `Compte M-PESA` ou `Compte ouvert`. Les décaissements, la dette créée et les positions de crédit ne sont plus affichés dans l'Extrait client, ses Word, PDF ou son classeur client.
- Les Word et PDF synthétisent six familles issues du parcours Turbo : dépôt normal, retrait, remboursement depuis M-PESA, remboursement depuis le compte ouvert, retour du capital DAT et entrée des intérêts DAT. Les cinq premières sont dérivées de Transactions Turbo; l'intérêt DAT constaté provient exclusivement de `Savings Account.interest_earned`.
- Les mouvements `Retrait Compte Bloque` sans ligne `MPESA ACCOUNT` sont présentés comme `Retours du capital mis en DAT`; ils ne sont pas ajoutés aux entrées 1441 ou aux sorties 15558 de l'extrait officiel.
...
- La situation DAT est datée depuis l'instantané `Savings Account`, puis à défaut depuis la dernière transaction Turbo du client; G2 n'intervient jamais dans ce calcul. Le taux DAT par défaut reste 11 %. L'intérêt des DAT en cours est une estimation de préparation et non une écriture comptable. Pour un DAT dénoué, seule l'entrée d'intérêt client réellement portée par `interest_earned` est affichée, sans part Vodacom ni colonnes techniques de traçabilité.
...
Dans `Importation et contrôle`, le tableau `Anomalies Transactions [Turbo]` affiche en première position une colonne épinglée `Raison de l'anomalie`. Elle précise chaque règle déclenchée et réunit les motifs lorsqu'une même écriture cumule plusieurs anomalies. Les filtres `statut` et `controle` s'appliquent à la synthèse et à cette liste; lorsque `controle` est filtré, seuls les motifs sélectionnés restent affichés. La valeur de chaque contrôle correspond au nombre de lignes techniques détaillables. Pour les écritures répétées, le nombre de groupes reste indiqué dans la colonne `detail`.

### Restitutions disponibles

Le sous-onglet `Projections` fournit un pilote explicable à court terme. Il estime, sur 7, 15, 30, 60 ou 90 jours, les créations de clients et de comptes, les nouveaux crédits, les remboursements, les opérations et les volumes à partir des historiques Solution Numérique. Le modèle combine le niveau des 28 derniers jours avec les derniers jours de semaine comparables, borne la tendance et mesure son erreur par test rétrospectif avec la `MAE — Mean Absolute Error` (`erreur absolue moyenne`) et le `WAPE — Weighted Absolute Percentage Error` (`erreur absolue pondérée en pourcentage`). Les montants CDF et USD restent séparés et le rapport G2 n'intervient pas dans l'apprentissage. L'écran comprend un mini-cours expliquant le machine learning, l'intervalle de prévision et les limites de lecture. L'échéancier DAT est présenté à part comme un calcul contractuel déterministe, et non comme une prédiction. Un avertissement est affiché lorsque l'historique est insuffisant, la dernière journée est partielle ou l'export Transactions contient exactement 100 000 lignes et peut être plafonné.
...
Les définitions opérationnelles des KPI M-PESA sont centralisées dans [`KPI_DICTIONARY.md`](KPI_DICTIONARY.md). Ce fichier précise la source, la formule, le traitement CDF/USD et les limites de chaque indicateur afin d'éviter les divergences entre l'écran, le Word et les exports Excel.

Le sous-onglet principal `Finance et comptabilité` réunit le pilotage financier et la comptabilité observée avec une période et un filtre de devises communs. Ses six volets sont `Vue direction`, `Flux et activité`, `Crédit, épargne et DAT`, `Balances et journaux`, `Risques et contrôles` et `Export`. Le volet `Balances et journaux` construit notamment les analyses financières observables directement dans Transactions M-PESA_Turbo :
...
Les volets de pilotage de `Finance et comptabilité` centralisent sur une période les analyses de microfinance et de contrôle démontrables depuis la Solution Numérique :
...
La Solution Numérique constitue l'unique source des montants, soldes, crédits, DAT, remboursements et alertes du cockpit. Le rapport G2 M-Pesa peut enrichir un nom et fournir une preuve de rapprochement dans les autres sous-onglets, mais n'intervient dans aucun calcul financier du pilotage. Les montants CDF et USD sont toujours séparés.
...
Le sous-onglet `Solution Numérique / M-Pesa` produit, depuis le rapport G2 M-Pesa lorsqu'il est chargé ou depuis le mode de repli Solution Numérique documenté ci-dessus :
...
- une répartition des statuts du rapport G2 par devise et fichier source lorsque G2 est disponible; en mode Solution Numérique seule, les opérations sont explicitement libellées `Comptabilisee Turbo` sans inférer un statut G2
...
Le sous-onglet `Perfect Client` produit trois populations inclusives au grain d'un téléphone normalisé :
...
Dans `Extrait client`, le classeur contient les feuilles utiles parmi `Synthese`, `Extrait_Turbo`, `Parcours_Turbo`, `DAT_En_Cours`, `Remboursements_Turbo`, `Elements_Extrait_Turbo`, `Interets_DAT_Credites`, `Comportement_Turbo`, `Mouvements_Internes`, `Controles_Client_Turbo`, `DAT_Final`, `G2_DAT` et `Diagnostics`. Il n'ajoute plus `Credit_Client_Turbo`, `Positions_Turbo`, l'ancienne feuille `Interets_DAT_Echus` ni `Credits`. `G2_DAT` est facultative et limitée au client sélectionné.

Dans `Solution Numérique / M-Pesa`, le classeur contient `Rapport_Journalier_Comptages`, `Rapport_Journalier_Synthese`, `Statuts_G2`, `Rapport_Journalier_Detail`, `Anomalies_G2`, `G2_DAT`, `Retention_Mensuelle` et `Retention_Detail`.
