...
Le sous-onglet `Prévisions Turbo` fournit un pilote explicable à court terme. Il estime, sur 7, 15, 30, 60 ou 90 jours, les créations de clients et de comptes, les nouveaux crédits, les remboursements, les opérations et les volumes à partir des historiques Turbo. Le modèle combine le niveau des 28 derniers jours avec les derniers jours de semaine comparables, borne la tendance et mesure son erreur par test rétrospectif avec la `MAE — Mean Absolute Error` (`erreur absolue moyenne`) et le `WAPE — Weighted Absolute Percentage Error` (`erreur absolue pondérée en pourcentage`). Les montants CDF et USD restent séparés et G2 n'intervient pas dans l'apprentissage. L'écran comprend un mini-cours expliquant le machine learning, l'intervalle de prévision et les limites de lecture. L'échéancier DAT est présenté à part comme un calcul contractuel déterministe, et non comme une prédiction. Un avertissement est affiché lorsque l'historique est insuffisant, la dernière journée est partielle ou l'export Transactions contient exactement 100 000 lignes et peut être plafonné.
