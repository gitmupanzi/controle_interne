...
| 119 | 9 | `119_cycle_money_provider_operations_money_provider_et_commissions_sur_la_periode` | Suivre les depots, retraits, transferts, commissions, frais et part agent Money Provider afin de piloter le canal mobile et ses revenus. | Rapport de synthese enrichi : operations par mois, source, type, devise, agent, client et compte provider, avec compteurs de controles qualite et motif d'anomalie. La requete couvre aussi `MOB_API_OPERATIONS` et force un plan adapte avec `OPTION (RECOMPILE)`. | Mensuel |
| 134 | 9 | `134_cycle_money_provider_streamlit` | Produire le fichier transactionnel detaille Money Provider pour les analyses du cycle money_provider et les controles operationnels. | Une ligne represente une operation mobile issue de `OPERATIONS_MOB_WS`, `MOB_OPERATIONS` ou `MOB_API_OPERATIONS`, enrichie avec client, telephone, agent, compte provider, frais observes, part agent, controle qualite et motif de controle. | A chaque televersement du cycle Money Provider |
